...
public abstract class MyApp : CGApplicationTemplate<CGApplication, Device, DeviceArgs>
{
    ...
    [DisplayNumericProperty(new[] {1d, 0.7, 1}, 0.1d, 1, "Цвет материала", 0, 1)]
    ...
    [DisplayNumericProperty(new[]{0d, 0, 0}, 0.1, 1, "Центр прозрачности")]
    public virtual DVector3 TransparencyPos { get; set; }
    ...
    private int uniformWatcherPos, uniformTransparencyPos;
    ...
    protected override void OnMainWindowLoad(object sender, EventArgs args)
    {
        ...
        AxesVertices = new[]
        {
            ...
            new Vertex(axisLen, 0, 0, 0, 0, 0, 1, 0f, 0), // Ox
            new Vertex(0, axisLen, 0, 0, 0, 0, 0, 1, 0), // Oy
            new Vertex(0, 0, axisLen, 0, 0, 0, 0, 0f, 1), // Oz
        };
        ...
        RenderDevice.AddScheduleTask((gl, s) =>
        {
            ...
            if ((uniformTransparencyPos = gl.GetUniformLocation(ProgShader, "TransparencyPos")) < 0)
            {
                throw new Exception("OpenGL error: не удалось найти переменную TransparencyPos");
            }
            
            if ((uniformTime = gl.GetUniformLocation(ProgShader, "time")) < 0)
            {
                throw new Exception("OpenGL error: не удалось найти переменную time");
            }
        });
        #endregion
    ...
    protected override void OnDeviceUpdate(object s, DeviceArgs e)
    {
        ...
        if (UseShader)
        {
            ...
            gl.Uniform3(uniformTransparencyPos, (float)TransparencyPos.X, (float)TransparencyPos.Y, (float)TransparencyPos.Z);
            gl.Uniform3(uniformWatcherPos, (float)CameraPosition.X, (float)CameraPosition.Y, (float)CameraPosition.Z);

            time += e.Delta;
            gl.Uniform1(uniformTime, time / 1000f);
        }
        else
        {
